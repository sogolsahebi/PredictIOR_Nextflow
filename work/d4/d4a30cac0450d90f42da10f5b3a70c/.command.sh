...
expr <- lapply(rda_files_path, function(file) {
    load(file)
    dat_icb
})

# Assign study names to the list elements
study_names <- substr(basename(list.files(path = 'data', pattern = '.rda')), 5, nchar(basename(list.files(path = 'data', pattern = '.rda'))) - 4)
