...
res_meta_pancancer <- lapply(1:length(expr), function(k) {
    metafun(
        coef = assoc.res$Coef[assoc.res$Cancer_type == cancer_types[k] & assoc.res$Treatment == treatment_types[k]], 
        se = assoc.res$SE[assoc.res$Cancer_type == cancer_types[k] & assoc.res$Treatment == treatment_types[k]],
        study  = assoc.res$Study[assoc.res$Cancer_type == cancer_types[k] & assoc.res$Treatment == treatment_types[k]], 
        pval = assoc.res$Pval[assoc.res$Cancer_type == cancer_types[k] & assoc.res$Treatment == treatment_types[k]], 
        n = assoc.res$N[assoc.res$Cancer_type == cancer_types[k] & assoc.res$Treatment == treatment_types[k]],
        cancer.type = cancer_types[k], 
        treatment = treatment_types[k],
        feature = "CXCL9"
    )
...
# Combine the meta-analysis results into one data frame
res_meta_pancancer <- do.call(rbind, res_meta_pancancer)
