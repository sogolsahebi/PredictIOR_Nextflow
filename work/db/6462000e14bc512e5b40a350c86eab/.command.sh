...
if (!is.null(res_meta_pertreatment$Other)) {
    ...
    res_meta_pertreatment <- data.frame(res_meta_pertreatment$`PD-1/PD-L1`$meta_summery)
}

# Save the results to a CSV file
write.csv(res_meta_pertreatment, file = "ICB_small_Mariathasan_meta_analysis_os_null_pertreatment.csv", row.names = FALSE)
