...
geneSig.score <- lapply(1:length(GeneSig_list), function(i) {
    ...
    if (method == "GSVA") { geneSig <- geneSigGSVA(dat.icb = ICB_small_Mariathasan, 
        ...
        geneSig <- geneSigMean(dat.icb = ICB_small_Mariathasan, sig = sig, sig.name = signature$Signature[i], missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, sig.perc = 0.8)
    } else if (method == "ssGSEA") {
        geneSig <- geneSigssGSEA(dat.icb = ICB_small_Mariathasan, sig = sig, sig.name = signature$Signature[i], missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, sig.perc = 0.8)
    } else if (method == "Specific Algorithm") {
        geneSig <- switch(sig_name,
                          "COX-IS_Bonavita" = geneSigCOX_IS(dat.icb = ICB_small_Mariathasan, sig = sig, sig.name = signature$Signature[i], missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, sig.perc = 0.8),
                          "IPS_Charoentong" = geneSigIPS(dat.icb = ICB_small_Mariathasan, sig = sig, sig.name = signature$Signature[i], missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, sig.perc = 0.8),
                          geneSig)
    } else {
...
print("Class of geneSig:")
print(class(geneSig))
print("Structure of geneSig:")
print(str(geneSig))


geneSig.score <- do.call(rbind, geneSig.score)
rownames(geneSig.score) <- substr(basename(GeneSig_list[i]), 1, nchar(basename(GeneSig_list[i]))-4)
