...
# Load all .rda files into the global environment and rename the 'sig' object to a unique name
GeneSig_list <- list.files(path = dir_GeneSig, pattern = '*.rda', full.names = TRUE)
lapply(GeneSig_list, function(file) {
    load(file)
    sig_name <- basename(GeneSig_list[i])
    assign(sig_name, sig, envir = .GlobalEnv)  # Assign 'sig' to a uniquely named object
})
