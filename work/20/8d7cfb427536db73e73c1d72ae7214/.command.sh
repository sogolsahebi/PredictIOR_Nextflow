...
assoc.res <- lapply(1:length(expr), function(k){
    if ('OS' == 'OS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 36, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'OS', cancer.type = cancer_types[k], treatment = treatment_types[k])
    } else if ('OS' == 'PFS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 24, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'OS', cancer.type = cancer_types[k], treatment = treatment_types[k])
    } else if ('OS' == 'Response') {
        geneLogReg(dat.icb = expr[[k]], missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], n0.cutoff = 3, n1.cutoff = 3, cancer.type = cancer_types[k], treatment = treatment_types[k])
    } else {
...
print("res_meta_percancer")
print(res_meta_percancer)
print(class(res_meta_percancer))

# Convert results to data frame ensuring consistent columns
res_meta_percancer_df_list <- lapply(res_meta_percancer, function(x) {
as.data.frame(matrix(unlist(x), ncol = length(x), byrow = TRUE))
})

# Combine the list of data frames into one data frame
res_meta_percancer_df <- do.call(rbind, res_meta_percancer_df_list)
