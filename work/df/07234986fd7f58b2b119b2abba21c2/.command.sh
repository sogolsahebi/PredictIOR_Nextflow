#!/usr/bin/env Rscript
    ...
    load(file_path)  # This assumes each .rda file contains one object, typically a data frame or list
    return(eval(as.name(basename(sub('*.rda', "", file_path))))  # Return the loaded object by name inferred from file name
})
