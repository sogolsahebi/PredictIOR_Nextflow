...
pattern <- ifelse('PFS' == 'OS', '_os', ifelse('PFS' == 'PFS', '_pfs', '_Response'))
...
write.csv(AllGeneSig_meta, file = "Meta_analysis_Sig_PFS_pancancer.csv", row.names = FALSE)
save(AllGeneSig_meta, file = "meta_pan_PFS.RData")
