...
assoc.res <- lapply(1:length(expr), function(k){
    if ('PFS' == 'OS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 36, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'PFS', cancer.type = cancer_types[k], treatment = treatment_types[k])
    } else if ('PFS' == 'PFS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 24, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'PFS', cancer.type = cancer_types[k], treatment = treatment_types[k])
...
write.csv(data.frame(res_meta_percancer ), file = "Meta_analysis_PFS_CXCL9_perncancer.csv", row.names = FALSE)
