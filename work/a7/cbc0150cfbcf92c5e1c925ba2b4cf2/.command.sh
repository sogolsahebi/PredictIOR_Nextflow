...
computeGeneSignature <- function(method, sig_name, sig) {
 ...
 # Load all .rda files 
lapply(list.files(path = 'SIG_data', pattern = '*.rda', full.names = TRUE), function(file) { load(file, envir = .GlobalEnv) })

# Create a list of the loaded objects using their actual names
loaded_objects <- basename(list.files(path = 'SIG_data', pattern = '*.rda', full.names = TRUE))
