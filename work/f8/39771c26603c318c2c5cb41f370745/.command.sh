...
computeGeneSignature <- function(method, sig_name, sig) {
    if (method == "GSVA") {
        ...
        geneSig <- get(method)(dat.icb = ICB_small_Padron, sig = sig, sig.name = sig_name, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, sig.perc = 0.8, study = "ICB_small_Padron")
    }
