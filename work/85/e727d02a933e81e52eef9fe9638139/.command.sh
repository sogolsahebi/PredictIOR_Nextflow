...
assoc.res <- lapply(1:length(expr), function(k){
    if ('Response' == 'OS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 36, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'Response', cancer.type = cancer_types[k], treatment = treatment_types[k])
    } else if ('Response' == 'PFS') {
        geneSurvCont(dat.icb = expr[[k]], time.censor = 24, missing.perc = 0.5, const.int = 0.001, n.cutoff = 15, feature = "CXCL9", study = names(expr)[k], surv.outcome = 'Response', cancer.type = cancer_types[k], treatment = treatment_types[k])
...
write.csv(data.frame(res_meta_pancancer), file = "Meta_analysis_Response_CXCL9_pancancer.csv", row.names = FALSE)
