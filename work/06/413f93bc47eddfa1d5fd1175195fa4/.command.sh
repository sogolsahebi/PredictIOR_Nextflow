...
dir_GeneSig <- './ICB_data'
GeneSig_list <- list.files(path = './ICB_data', pattern = '*.rda', full.names = TRUE)
...
if (length(GeneSig_list) > 0) {
    # Example: Load the first .rda file
    load(GeneSig_list[1])

    # Further processing using the loaded data
    # ...
} else {
    print("No .rda files found in the directory.")
}
