...
geneSig.score <- read.csv("ICB_small_Riaz_GeneSigScore.csv", row.names = 1)
...
res.logreg <- lapply(1:nrow(geneSig.score), function(k){
    ...
    res <- geneSigLogReg(dat.icb = ICB_small_Riaz,
                        geneSig = geneSig_vector,
                        n.cutoff = 10,
                        study =  "ICB_small_Riaz",
                        sig.name = sig_name,
                        n0.cutoff = 3, 
                        n1.cutoff = 3,
...
# Save as CSV file
write.csv(res.logreg, file = "ICB_small_Riaz_GeneSig_Response.csv", row.names = TRUE)
