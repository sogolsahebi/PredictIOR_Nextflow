...
res_meta_percancer_df <- do.call(rbind, lapply(res_meta_percancer, function(x) {
if (lengths(x) == lengths(res_meta_percancer[[1]])) {
    return(data.frame(x))
} else {
    return(data.frame(matrix(ncol = length(res_meta_percancer[[1]]), nrow = 0)))
}
}))
