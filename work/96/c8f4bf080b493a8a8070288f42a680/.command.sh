...
# Load RDA files
list_expr <- lapply(list.files(path = './data', pattern = '*.rda', full.names = TRUE), function(file) {
    load(file)
})
...
assoc.res <- lapply(1:length(list_expr), function(k){
    geneSurvCont(dat.icb = get(list_expr[[k]]),
                 time.censor = 36,
                 missing.perc = 0.5,
                 const.int = 0.001,
                 n.cutoff = 15,
